...
async function onNarrateText(args, text) {
    ...
    const name = (['SillyTavern System', 'Wulf\'s Hollow System'].includes(baseName) ? DEFAULT_VOICE_MARKER : baseName) || DEFAULT_VOICE_MARKER;
...
export function getCharacters(unrestricted) {
    ...
    if (extension_settings.tts.multi_voice_enabled) {
        ...
        for (const char of characters) {
            if (char === DEFAULT_VOICE_MARKER || char === 'SillyTavern System' || char === 'Wulf\'s Hollow System') {
...
async function initVoiceMapInternal(unrestricted) {
    ...
    for (const character of characters) {
        if (character === 'SillyTavern System' || character === 'Wulf\'s Hollow System') {
            continue;
        }
