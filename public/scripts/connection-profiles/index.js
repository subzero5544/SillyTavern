...
import { ConnectionManagerRequestService } from './request-service.js';

const NONE = '<None>';
const EMPTY = '<Empty>';
const PROMPT_ENTRIES_KEY = 'prompt-entries';
...
const FANCY_NAMES = {
    ...
    [PROMPT_ENTRIES_KEY]: 'Prompt Entries',
};

/**
 * Renders a native connection profile template.
 * @param {string} templateId Template ID
 * @param {object} templateData Additional data to pass to the template
 * @returns {Promise<string>} Rendered HTML
 */
function renderConnectionProfileTemplateAsync(templateId, templateData = {}) {
    return renderTemplateAsync(`/scripts/connection-profiles/${templateId}.html`, templateData, true, true, true);
}
 ...
 * @property {PromptEntriesProfileState} [prompt-entries] Prompt Manager enabled entry state
 * @property {string[]} [exclude] Commands to exclude
 */

/**
 * @typedef {Object} PromptEntriesProfileState
 * @property {string} [preset] Settings Preset used when the state was captured
 * @property {{identifier: string, name?: string}[]} enabled Enabled Prompt Manager entries
 */
...
/**
 * Captures enabled Chat Completion Prompt Manager entries into a connection profile.
 * Prompt identifiers are authoritative; names are stored only for display and troubleshooting.
 * @param {ConnectionProfile} profile Connection profile
 * @param {string[]} excludeList Profile exclude list
 */
function capturePromptEntries(profile, excludeList) {
    if (excludeList.includes(PROMPT_ENTRIES_KEY)) {
        delete profile[PROMPT_ENTRIES_KEY];
        return;
    }

    if (!promptManager) {
        console.warn('Cannot capture connection profile prompt entries: Prompt Manager is not initialized.');
        return;
    }

    const promptOrder = promptManager.getPromptOrderForCharacter(promptManager.activeCharacter);
    const enabled = promptOrder
        .filter(entry => entry?.enabled === true && typeof entry?.identifier === 'string')
        .map(entry => {
...
/**
 * Updates Prompt Manager entry toggles to exactly match a saved profile state.
 * @param {ConnectionProfile} profile Connection profile
 * @returns {Promise<void>}
 */
async function applyPromptEntries(profile) {
    const savedPromptEntries = profile?.[PROMPT_ENTRIES_KEY];
    if (profile?.mode !== 'cc' || !savedPromptEntries) {
        return;
    }

    if (!promptManager) {
        console.warn('Cannot apply connection profile prompt entries: Prompt Manager is not initialized.');
        return;
    }

    if (!Array.isArray(savedPromptEntries.enabled)) {
        console.warn('Cannot apply connection profile prompt entries: saved state is invalid.', savedPromptEntries);
        return;
    }

    const promptOrder = promptManager.getPromptOrderForCharacter(promptManager.activeCharacter);
    const currentIdentifiers = new Set(promptOrder.map(entry => entry?.identifier).filter(Boolean));
    const enabledIdentifiers = new Set(savedPromptEntries.enabled.map(entry => entry?.identifier).filter(Boolean));
    const tokenCounts = promptManager.tokenHandler?.getCounts?.();
    ...
    for (const entry of promptOrder) {
        ...
        const enabled = enabledIdentifiers.has(entry.identifier);
        if (entry.enabled !== enabled && tokenCounts) {
            tokenCounts[entry.identifier] = null;
        }
        entry.enabled = enabled;
    }

    const missingEntries = savedPromptEntries.enabled
        .filter(entry => entry?.identifier && !currentIdentifiers.has(entry.identifier))
        .map(entry => entry.name && entry.name !== entry.identifier ? `${entry.name} (${entry.identifier})` : entry.identifier);

    if (missingEntries.length > 0) {
        console.warn('Connection profile prompt entries were not found in the current preset:', missingEntries);
    }
...
/**
 * Formats saved Prompt Manager entries for display.
 * @param {ConnectionProfile} profile Connection profile
 * @returns {string|null} Display value, or null if the profile has no saved Prompt Manager state
 */
function formatPromptEntriesForDisplay(profile) {
    const savedPromptEntries = profile?.[PROMPT_ENTRIES_KEY];
    if (!savedPromptEntries || !Array.isArray(savedPromptEntries.enabled)) {
        return null;
    }

    if (savedPromptEntries.enabled.length === 0) {
        return NONE;
    }

    return savedPromptEntries.enabled
        .map(entry => {
            if (!entry?.identifier) {
                return null;
            }
            return entry.name && entry.name !== entry.identifier ? `${entry.name} (${entry.identifier})` : entry.identifier;
        })
        .filter(Boolean)
        .join(', ');
}
...
/**
 * Gets the selected Chat Completion preset name from the preset select element.
 * @returns {string} Selected preset name
 */
function getSelectedChatCompletionPresetName() {
    /** @type {HTMLSelectElement|null} */
    const presetSelect = document.getElementById('settings_preset_openai');
    return presetSelect?.selectedOptions?.[0]?.textContent ?? '';
}

/**
 * Creates a cancellable waiter for a Chat Completion preset change.
 * @param {string} presetName Expected preset name
 * @returns {{promise: Promise<boolean>, cancel: () => void}}
 */
function createPresetChangeWaiter(presetName) {
    let cleanup = () => { };
    let resolveWait = (_value) => { };

    const promise = new Promise(resolve => {
        resolveWait = resolve;

        const listener = (data) => {
            if (data?.apiId && data.apiId !== 'openai') {
                return;
            }

            if (presetName && data?.name && data.name !== presetName) {
                return;
            }
        ...
        const timeoutId = setTimeout(() => {
            cleanup();
            console.warn('Timed out waiting for Chat Completion preset to finish applying before restoring profile prompt entries:', presetName);
            resolve(false);
        }, 5000);
...
async function readProfileFromCommands(mode, profile, cleanUp = false) {
    ...
    if (mode === 'cc') {
        capturePromptEntries(profile, excludeList);
    } else {
        delete profile[PROMPT_ENTRIES_KEY];
    }
}
...
async function createConnectionProfile(forceName = null) {
    ...
    const template = $(await renderConnectionProfileTemplateAsync('profile', { profile: profileForDisplay }));
...
function makeFancyProfile(profile) {
    return Object.entries(FANCY_NAMES).reduce((acc, [key, value]) => {
        if (key === PROMPT_ENTRIES_KEY) {
            const promptEntries = formatPromptEntriesForDisplay(profile);
            if (promptEntries !== null) {
                acc[value] = promptEntries;
            }
            return acc;
        }
...
async function applyConnectionProfile(profile) {
    ...
    try {
        for (const command of commands) {
            ...
            const shouldWaitForPresetChange = mode === 'cc'
                && command === 'preset'
                && profile[PROMPT_ENTRIES_KEY]
                && argument
                && getSelectedChatCompletionPresetName() !== argument;
            const presetChangeWaiter = shouldWaitForPresetChange ? createPresetChangeWaiter(argument) : null;
            let result = null;

            try {
                const args = getNamedArguments(allowEmpty ? { force: 'true' } : {});
                result = await SlashCommandParser.commands[command].callback(args, argument);
            } catch (error) {
                presetChangeWaiter?.cancel();
                console.error(`Failed to execute command: ${command} ${argument}`, error);
            }

            if (presetChangeWaiter) {
                if (String(result ?? '').toLowerCase().trim() === String(argument).toLowerCase().trim()) {
                    await presetChangeWaiter.promise;
                } else {
                    presetChangeWaiter.cancel();
                }
            }
        ...
        await applyPromptEntries(profile);
    } finally {
        spinner.stop();
    }
}
...
async function renderDetailsContent(detailsContent) {
    ...
    if (profile) {
        ...
        const template = await renderConnectionProfileTemplateAsync('view', templateParams);
...
export async function init() {
    ...
    if (document.getElementById('connection_profiles')) {
        initialized = true;
        return;
    }

    const container = document.getElementById('rm_api_block');
    const settings = await renderConnectionProfileTemplateAsync('settings');
    ...
    editButton.addEventListener('click', async () => {
        ...
        const commands = profile.mode === 'cc' ? [...CC_COMMANDS, PROMPT_ENTRIES_KEY] : TC_COMMANDS;
        ...
        const template = $(await renderConnectionProfileTemplateAsync('edit', { name: profile.name, settings }));
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        helpString: `
            ...
            <div>
                ${t`Uses the currently selected connection profile or the specified profile= argument.`}
            </div>
            <div>
